<template>
	<view class="page">
		...
		<view class="content">
			<view class="brand" v-for="(item,index) in brandsList" :key="index"
				@click="$toUrl('/pages/index/brand/brand',{id:item.id})">
				<imageBox width="100%" height="400rpx" :imageUrl="item.choice_image"></imageBox>
				<view class="info">{{item.brand_name}}</view>
			</view>
		</view>
		...
		methods: {
			...
			loadMore() {
				...
				this.$api.index.CHOICENESS({
					...
					this.brandsList = this.brandsList.concat(res.data.list);
